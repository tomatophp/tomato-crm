     ...
     @foreach($model->accountRequestMetas as $meta)
         <x-splade-data :default="['is_rejected' => $meta->is_rejected]">
             <div class="col-span-2 flex justify-between">
                 <div>
                     @if($meta->model_id)
                         <x-tomato-admin-row :label="str($meta->key)->replace('_',' ')->title()" :value="$meta->model_type::find($meta->model_id)?->name" type="text" />
                     @elseif($meta->value === 'image')
                         <x-tomato-admin-row :label="str($meta->key)->replace('_',' ')->title()" :value="$meta->getMedia('image')->first()?->getUrl()" type="image" />
                     @else
                         <x-tomato-admin-row :label="str($meta->key)->replace('_',' ')->title()" :value="$meta->value" type="string" />
                     @endif
                 </div>
                 <div>
                     @if(!$meta->is_approved)
             ...
             <div v-if="data.is_rejected" class="col-span-2 w-full">
                 <x-splade-form :default="$meta" method="POST" class="flex flex-col gap-4" action="{{route('admin.account-requests.reject', $meta->id)}}">
