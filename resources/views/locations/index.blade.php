<x-tomato-admin-layout>
    ...
    <x-slot name="headerBody">
        <Link slideover href="/admin/locations/create" class="filament-button inline-flex items-center justify-center py-1 gap-1 font-medium rounded-lg border transition-colors focus:outline-none focus:ring-offset-2 focus:ring-2 focus:ring-inset dark:focus:ring-offset-0 min-h-[2.25rem] px-4 text-sm text-white shadow focus:ring-white border-transparent bg-primary-600 hover:bg-primary-500 focus:bg-primary-700 focus:ring-offset-primary-700 filament-page-button-action">
            {{trans('tomato-admin::global.crud.create-new')}} {{__('Location')}}
        </Link>
    </x-slot>
    ...
    <div class="pb-12" v-cloak>
        <div class="mx-auto">
            @if(\TomatoPHP\TomatoCrm\Models\Location::count())
            <x-splade-table :for="$table" striped>
                <x-splade-cell actions>
                    <div class="flex justify-start">
                        <Link href="/admin/locations/{{ $item->id }}" class="px-2 text-blue-500" slideover>
                            <div class="flex justify-start space-x-2">
                                <x-heroicon-s-eye class="h-4 w-4 ltr:mr-2 rtl:ml-2"/>
                                <span>{{trans('tomato-admin::global.crud.view')}}</span>
                            </div>
                        </Link>
                        <Link href="/admin/locations/{{ $item->id }}/edit" class="px-2 text-yellow-400" slideover>
                            <div class="flex justify-start space-x-2">
